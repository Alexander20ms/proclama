...
export default function ReactionBar({ proclamaId, initialReacciones }: Props) {
  const [reacciones, setReacciones] = useState<Record<string, number>>(initialReacciones || {});
  ...
  if (total === 0 && mias.length === 0) {
    // Show buttons even with 0 counts
  }

  return (
    <div className="flex items-center gap-1 flex-wrap mt-3 pt-3 border-t border-[#1E1E1E]">
      {EMOJIS.map((emoji) => {
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-sm transition-all select-none
              ${reacted
                ...
                : "bg-[#1E1E1E] text-[#A0A0A0] hover:bg-[#2A2A2A] hover:text-white border border-transparent"
              }
              ${animating === emoji ? "animate-pop" : ""}
            `}
          >
            <span className={animating === emoji ? "scale-125 inline-block" : "inline-block"}>
              {emoji}
            </span>
            {count > 0 && (
